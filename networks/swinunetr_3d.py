...
class SwinTransformerBlock3D(nn.Module):
    ...
    def loadFrom(self, weights, n_block, layer):
        ROOT = f"module.{layer}.0.blocks.{n_block}."
        block_names = ['norm1.weight', 'norm1.bias', 'attn.relative_position_bias_table',
                       'attn.relative_position_index', 'attn.qkv.weight', 'attn.qkv.bias', 'attn.proj.weight',
...
if __name__ == "__main__":
    net_3d = SwinUNETR(img_size=(64, 64, 64), in_channels=1, out_channels=1).cuda()
    t1 = torch.rand(1, 1, 64, 64, 64).cuda()
    out = net_3d(t1)
    print(out.shape)
