...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # x0 = x[0]
        # Head Part
        height, width = x[0].size(2)*4, x[0].size(3)*4
...
class HRNet_LDAC(nn.Module):

    def __init__(self, config, **kwargs):
        self.inplanes = 64
        extra = config.MODEL.EXTRA
        super(HRNet_LDAC, self).__init__()

        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc_ldac = nn.Linear(256, 2)
        # stem net
        self.conv1 = nn.Conv2d(1, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn1 = BatchNorm2d(64, momentum=BN_MOMENTUM)
    ...
    def forward(self, x):
        ...
        x_ac = self.pool(x).reshape(x.shape[0], -1)
        ac = self.fc_ldac(x_ac)
...
class HRNet_BCS(nn.Module):

    def __init__(self, config, **kwargs):
        self.inplanes = 64
        extra = config.MODEL.EXTRA
        super(HRNet_BCS, self).__init__()
        ...
        self.mask_head = nn.Sequential(
            nn.Conv2d(
                ...
                stride=1,
                padding=1 if extra.FINAL_CONV_KERNEL == 3 else 0),
            BatchNorm2d(2, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=2,
                out_channels=1,
...
_C.MODEL.PRETRAINED = ''
_C.MODEL.NUM_JOINTS = 2
_C.MODEL.TARGET_TYPE = 'Gaussian'
_C.MODEL.IMAGE_SIZE = [144, 144]  # width * height
_C.MODEL.HEATMAP_SIZE = [72, 72]  # width * height
_C.MODEL.SIGMA = 1.5
_C.MODEL.EXTRA = CN()
